...
{
    public partial class Purchase_Return_Help : Form
    {
        ...
        public Purchase_Return_Help(int i, string docType)
        {
            InitializeComponent();
            if (i == 0)
            {
        ...
        private void Sales_Return_Help_Load(object sender, EventArgs e)
        {
            try
            {
                //cmd.CommandText = "SELECT  CONVERT(NVARCHAR, DOC_ID) AS DOC_ID,DOC_NO,CONVERT(NVARCHAR,DOC_DATE_GRE,103) AS DOC_DATE_GRE,DOC_DATE_HIJ,DOC_REFERENCE,CURRENCY_CODE,SUPPLIER_CODE,SalesMan,NOTES,CONVERT(NVARCHAR,TOTAL_FOB_AMOUNT) AS TOTAL_AMOUNT,GROSS,TAX_TOTAL,NET_VAL,PAY_CODE,CARD_NO FROM INV_PURCHASE_HDR WHERE DOC_TYPE != '" + type + "'";       //and DOC_TYPE!='SAL.CREDITNOTE'       
                //adapter.Fill(table);
                help.Type = type;
                table = help.getAllPurchaseVoucher();
                ...
                c["SUPPLIER_CODE"].HeaderText = "Supplier Code";
                c["SalesMan"].HeaderText = "Salesman Code";
                c["NOTES"].HeaderText = "Notes";
                c["TAX_TOTAL"].HeaderText = "Tax Amount";
               
                c["TOTAL_AMOUNT"].HeaderText = "Total Amount";
                c["NET_VAL"].HeaderText = "Nett Amount";
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (dgItems.Rows.Count > 0 && dgItems.CurrentRow != null && MessageBox.Show("Are you sure?", "Stock Deletion", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    ...
                    //conn.Open();
                    //cmd.CommandText = "DELETE FROM INV_PURCHASE_HDR WHERE DOC_NO = '" + dgItems.CurrentRow.Cells[0].Value + "';DELETE FROM INV_SALES_DTL WHERE DOC_NO = '" + dgItems.CurrentRow.Cells[0].Value + "'";
                    //cmd.ExecuteNonQuery();
