 ...
 public  class clsHelper
    {
     ...
     {
         string Query = "SELECT DOC_NO,CONVERT(NVARCHAR,DOC_DATE_GRE,103) AS DOC_DATE_GRE,DOC_DATE_HIJ,SUP_CODE,CONVERT(NVARCHAR,AMOUNT) AS AMOUNT,PAY_CODE,CHQ_NO,CONVERT(NVARCHAR,CHQ_DATE,103) AS CHQ_DATE,BANK_CODE,CUR_CODE,NOTES,TOTAL_PAID,TOTAL_CURRENT,TOTAL_BALANCE, DEBIT_CODE, CREDIT_CODE FROM PAY_PAYMENT_VOUCHER_HDR h inner join tbl_financialyear f on Convert(Varchar, h.DOC_DATE_GRE, 111)  between f.SDate and f.EDate and f.CurrentFY = 1  ";
         return DbFunctions.GetDataTable(Query);
     }
     public DataTable getAllReceiptVouchers()
     {
         string Query = "SELECT DOC_NO, DOC_DATE_GRE,DOC_DATE_HIJ,CUST_CODE AS SUP_CODE,CONVERT(NVARCHAR,AMOUNT) AS AMOUNT,PAY_CODE,CHQ_NO,CONVERT(NVARCHAR,CHQ_DATE,103) AS CHQ_DATE,BANK_CODE,CUR_CODE,NOTES,TOTAL_PAID,TOTAL_CURRENT,TOTAL_BALANCE, DEBIT_CODE, CREDIT_CODE,DISCOUNT FROM REC_RECEIPTVOUCHER_HDR h inner join tbl_financialyear f on Convert(Varchar, h.DOC_DATE_GRE, 111) between f.SDate and f.EDate and f.CurrentFY = 1 ";
     ...
     {
         string Query = "SELECT  CONVERT(NVARCHAR, DOC_ID) AS DOC_ID,DOC_NO,CONVERT(NVARCHAR,DOC_DATE_GRE,103) AS DOC_DATE_GRE,DOC_DATE_HIJ,PUR_TYPE,DOC_REFERENCE,CURRENCY_CODE,SUPPLIER_CODE,SalesMan,NOTES,CONVERT(NVARCHAR,TOTAL_FOB_AMOUNT) AS TOTAL_AMOUNT,GROSS,TAX_TOTAL,NET_VAL,PAY_CODE,CARD_NO FROM INV_PURCHASE_HDR h inner join tbl_financialyear f on Convert(Varchar, h.DOC_DATE_GRE, 111) between f.SDate and f.EDate and f.CurrentFY = 1 WHERE DOC_TYPE  in('" + type + "')";       //and DOC_TYPE!='SAL.CREDITNOTE'       
         return DbFunctions.GetDataTable(Query);
     }
     ...
     {
         string Query = "SELECT DOC_NO,CONVERT(NVARCHAR,DOC_DATE_GRE,103) AS C,DOC_DATE_HIJ,DOC_REFERENCE,SUPPLIER_CODE,NOTES,PAY_CODE,CARD_NO,TAX_TOTAL,CESS_AMOUNT,GROSS,DISCOUNT_VAL,NET_VAL,SUP_INV_NO,FREIGHT_AMT FROM INV_PURCHASE_HDR h inner join tbl_financialyear f on h.DOC_DATE_GRE between f.SDate and f.EDate and f.CurrentFY = 1 where DOC_TYPE<>'LGR.PRT' ";
         return DbFunctions.GetDataTable(Query);
     }
     public DataTable getAllPurchaseReturnVoucherInPurHelper()
     {
         string Query = "SELECT  CONVERT(NVARCHAR, DOC_ID) AS DOC_ID,DOC_NO,CONVERT(NVARCHAR,DOC_DATE_GRE,103) AS DOC_DATE_GRE,DOC_DATE_HIJ,DOC_REFERENCE,CURRENCY_CODE,SUPPLIER_CODE,SalesMan,NOTES,CONVERT(NVARCHAR,TOTAL_FOB_AMOUNT) AS TOTAL_AMOUNT,GROSS,TAX_TOTAL,NET_VAL,PAY_CODE,CARD_NO,DEBIT_NOTE FROM INV_PURCHASE_HDR h inner join tbl_financialyear f on Convert(Varchar, h.DOC_DATE_GRE, 111) between f.SDate and f.EDate and f.CurrentFY = 1 WHERE DOC_TYPE  in('" + type + "')";       //and DOC_TYPE!='SAL.CREDITNOTE'       
         return DbFunctions.GetDataTable(Query);
     }
     public DataTable getAllSalesVoucher()
     {
         string Query = "SELECT  CONVERT(NVARCHAR, DOC_ID) AS DOC_ID,DOC_NO,SALE_TYPE,CONVERT(NVARCHAR,DOC_DATE_GRE,103) AS DOC_DATE_GRE,DOC_DATE_HIJ,DOC_REFERENCE,CURRENCY_CODE,CUSTOMER_CODE,CUSTOMER_NAME_ENG,SALESMAN_CODE,NOTES,CONVERT(NVARCHAR,TOTAL_AMOUNT) AS TOTAL_AMOUNT,TAX_TOTAL,VAT,DISCOUNT,NET_AMOUNT,PAY_CODE,CARD_NO FROM INV_SALES_HDR h inner join tbl_financialyear f on Convert(Varchar, h.DOC_DATE_GRE, 111) between f.SDate and f.EDate and f.CurrentFY = 1 WHERE DOC_TYPE not in('" + type + "')";       //and DOC_TYPE!='SAL.CREDITNOTE'       
         return DbFunctions.GetDataTable(Query);
     }
